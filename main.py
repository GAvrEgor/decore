...
def path(file_path):
    def decor(function):
        def new_func(*args, **kwargs):
            date = datetime.datetime.now().strftime('%d.%m.%y time %H:%M:%S')
            name = function.__name__
            arg = f'Данные на вход {args}, {kwargs}'
            out = function(*args, **kwargs)
            result = f'Функция {name} {arg} вызвана {date} \n Получен результат {out}\n ***********************************************\n'
